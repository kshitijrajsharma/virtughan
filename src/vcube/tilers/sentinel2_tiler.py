...
from .tiler_common import TileCommon

matplotlib.use("Agg")


class TileProcessor(TileCommon):
    """
    Processor for generating and caching Sentinel-2 tiles.
    """
    ...
    async def cached_generate_tile(
        ...
        results = await search_stac_api_async(
            bbox_geojson, start_date, end_date, cloud_cover, collection="sentinel-2-l2a"
        )
        ...
        if latest:
            if len(results) > 0:
                ...
                # print(f"Sentinel URL: {band1_url}")
